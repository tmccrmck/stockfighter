...
def start():
	global level_id, level_id, my_account, venue, target_stock
	level_id = 'dueling_bulldozers'
	json = api.gm_start(level_id)
	my_account = json['account']
	venue = json['venues'][0]
	target_stock = json['tickers'][0]
...
def basic_strategy():
	...
	while nav < 25000000:
		...
		if json['bids']:
			...
			print("Best bid: %s. Selling if > %.2f with multiplier %.2f" % (bestPrice, priceNeeded, sell_multiplier))
		...
		if json['asks']:
			...
			print("Best sell: %s. Buying if < %.2f with multiplier %.2f" % (bestPrice, priceNeeded, buy_multiplier))
			if bestPrice < priceNeeded and int(qty) != 0:
